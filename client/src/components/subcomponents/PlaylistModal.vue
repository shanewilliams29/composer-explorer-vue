...
<div>
  <b-modal id="playlist-modal" @show="resetForm" hide-footer>
    ...
    <div class="m-2 text-center">
      ...
      <div v-if="$view.playlistError" style="color: darkred;">Sorry, we encounted the following error:<br>"{{$view.playlistError}}".<br>
        <b-button class="mt-4" @click="$bvModal.hide('playlist-modal')">Close</b-button></div>
      <div v-if="$view.playlistSuccess" style="color: darkgreen;">Success! Tracks added to playlist "{{form.name}}".<br>
      <b-button class="mt-4" @click="$bvModal.hide('playlist-modal')">Close</b-button></div>
